...
import {
    CornerDownRightIcon,
    CircleCheckIcon, ClockArrowUpIcon,
    CircleXIcon, ClockFadingIcon,
    LoaderIcon
} from "lucide-react";
...
export const columns: ColumnDef<MeetingGetMany[number]>[] = [
    {
        ...
        cell: ({ row }) => (
            <div className="flex gap-y-1 flex-col">
                <span className="capitalized font-semibold">{row.original.name}</span>
...

    {
        ...
        cell: ({ row }) => {
            ...
            cell: ({row}) => (
                <Badge variant="outline"
                className="capitalized [&>svg]:size-4 flex items-center gap-x-2">
